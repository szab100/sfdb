...
#ifndef UTIL_TYPES_INTEGRAL_TYPES_H_
#define UTIL_TYPES_INTEGRAL_TYPES_H_

#include <limits.h>  // not climits; this file is used from C
#include <stdint.h>  // not cstdint; this file is used from C

// Preprocessor symbol that is defined when [u]int64 and [u]int64_t name the
// same types.
//
// After migration, int64 will be changed to a typedef of int64_t, and this will
// always be true.  Then we will migrate all instances of int64 to int64_t, and
// remove this macro.
//
// Implementation: If long is a 32-bit type, then assume int64_t is long long,
// and otherwise assume int64_t is long. 
#if (LONG_MAX == 2147483647) || (defined(__APPLE__) && defined(__MACH__))
#define INT64_IS_INT64_T 1
#endif
...
#ifndef SWIG
// Standard typedefs
// Don't use 'schar'. In the rare case you need this, use 'signed char'.
...
#ifdef INT64_IS_INT64_T
static_assert(std::is_same<int64, int64_t>::value,
              "INT64_IS_INT64_T is set but should not be.");
static_assert(std::is_same<uint64, uint64_t>::value,
              "INT64_IS_INT64_T is set but should not be.");
#else
static_assert(!std::is_same<int64, int64_t>::value,
              "INT64_IS_INT64_T is not set but should be.");
static_assert(!std::is_same<uint64, uint64_t>::value,
              "INT64_IS_INT64_T is not set but should be.");
#endif  // INT64_IS_INT64_T

#endif  // defined(__cplusplus) && !defined(SWIG)

#endif  // UTIL_TYPES_INTEGRAL_TYPES_H_
